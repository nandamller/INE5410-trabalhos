...
stats_t play(cell_t **board, cell_t **newboard, int size, int n_threads)
{
    ...
    for (i = 0; i < n_threads; i++)
    {
        ...
        args->i = i*(ceil(size/n_threads));
        args->f = i*(ceil(size/n_threads)) + ceil(size/n_threads);
        if (i==n_threads-1) args->f = size;
        args->size = size;
        args->stats = stats;
        args->board = board;
        args->newboard = newboard;
        pthread_create(&threads[i], NULL, thread_play, (void *)args);
    }
